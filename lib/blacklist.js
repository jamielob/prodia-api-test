    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: `Replace copyrighted brands/characters with "abstract [description]". Examples: "BMW"→"abstract luxury car", "Spider-Man"→"abstract spider superhero", "Harry Potter"→"abstract young wizard", "Nike"→"abstract athletic shoes". Return ONLY the filtered text, NO explanations.
...
Filtered:`
    ...
    // Remove explanation text (e.g., "Since there are no...")
    if (filtered && filtered.includes('Since ')) {
      // Try to extract just the filtered text before the explanation
      const parts = filtered.split(/Since |As there|There are no/);
      if (parts[0].trim()) {
